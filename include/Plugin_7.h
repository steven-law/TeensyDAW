...
public:
    ...
    void draw_plugin()
    {
        ...
        if (change_plugin_row)
        {
            ...
            drawPot(0, 1, potentiometer[4], "Trshold");
            drawPot(1, 1, potentiometer[5], "Attack");
            drawPot(2, 1, potentiometer[6], "Release");
            drawPot(3, 1, potentiometer[7], "Hysteris");

            drawPot(0, 2, potentiometer[8], "Ratio");
            drawPot(1, 2, potentiometer[9], "Knewdth");
            drawPot(2, 2, potentiometer[10], "AmakUpG");
            drawPot(3, 2, potentiometer[11], "makUpG");

            drawPot(3, 3, potentiometer[15], "Gain");
    ...
    void set_dynamics_threshold(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            threshold = (float)map(potentiometer[n], 0, 127, MIN_DB, MAX_DB);
            dynamics.gate(threshold, attack, release, hysterisis);              // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 0.0f-6.0f
            dynamics.compression(threshold, attack, release, ratio, kneeWidth); // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 1.0f-60.0f ;; 0.0f-32.0f
            dynamics.limit(threshold, attack, release);                         // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 0.0f-6.0f
        }
    ...
    void set_dynamics_attack(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            attack = (float)map(potentiometer[n], 0, 127, MIN_T, MAX_T);
            dynamics.gate(threshold, attack, release, hysterisis);              // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 0.0f-6.0f
            dynamics.compression(threshold, attack, release, ratio, kneeWidth); // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 1.0f-60.0f ;; 0.0f-32.0f
            dynamics.limit(threshold, attack, release);                         // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 0.0f-6.0f
        }
    ...
    void set_dynamics_release(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            release = (float)map(potentiometer[n], 0, 127, MIN_T, MAX_T);
            dynamics.gate(threshold, attack, release, hysterisis);              // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 0.0f-6.0f
            dynamics.compression(threshold, attack, release, ratio, kneeWidth); // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 1.0f-60.0f ;; 0.0f-32.0f
            dynamics.limit(threshold, attack, release);                         // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 0.0f-6.0f
        }
    ...
    void set_dynamics_hysterisis(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            hysterisis = (float)map(potentiometer[n], 0, 127, 0.0f, 6.0f);
            dynamics.gate(threshold, attack, release, hysterisis); // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 0.0f-6.0f
        }
    }
    ...
    void set_dynamics_ratio(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            ratio = (float)map(potentiometer[n], 0, 127, RATIO_OFF, RATIO_INFINITY);
            dynamics.compression(threshold, attack, release, ratio, kneeWidth); // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 1.0f-60.0f ;; 0.0f-32.0f
        }
    }
    void set_dynamics_kneeWidth(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            kneeWidth = (float)map(potentiometer[n], 0, 127, 0.0f, 32.0f);
            dynamics.compression(threshold, attack, release, ratio, kneeWidth); // float -110.0f-0.0f ;; 0.03f-4.00f ;; 0.03f-4.00f ;; 1.0f-60.0f ;; 0.0f-32.0f
        }
    }
    void set_dynamics_makeupGain(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            float sustain = (float)map(potentiometer[n], 0, 127, -12.0f, 24.0f);
    ...
    void set_dynamics_autoMakeupGain(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            float sustain = (float)map(potentiometer[n], 0, 127, 0.0f, 60.0f);
    ...
    void set_amp_gain(byte XPos, byte YPos, const char *name)
    {
        if (enc_moved[XPos])
        {
            int n = XPos + (YPos * NUM_ENCODERS);
            potentiometer[n] = getEncodervalue(XPos, YPos, name, potentiometer[n]);
            float sustain = (float)map(potentiometer[n], 0, 127, 0.0f, 6.0f);
